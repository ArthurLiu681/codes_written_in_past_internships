...
wind_info = pandas.read_excel(".\\Data - Part 1.xlsx", sheet_name="Wind") \
    .set_index(["Start Date", "Start Time 30 Minute Period"]).fillna(value=0)
demand_info = pandas.read_excel(".\\Data - Part 1.xlsx", sheet_name="Demand") \
    .set_index(["Start Date", "Start Time 30 Minute Period"]).fillna(value=0)
price_info = pandas.read_excel(".\\Data - Part 1.xlsx", sheet_name="Actual Prices") \
...
# forecast error change rate factor
wind_info["average_wind_forecasts"] = [numpy.mean([forecast for forecast in list(wind_info.iloc[i])[-4:] if forecast != 0]) for i in range(len(wind_info.index))]
wind_info["wind_forecast_error"] = list(wind_info["Actual Wind (MW)"] - wind_info["average_wind_forecasts"])

demand_info["average_demand_forecasts"] = [numpy.mean([forecast for forecast in list(demand_info.iloc[i])[-4:] if forecast != 0]) for i in range(len(demand_info.index))]
demand_info["demand_forecast_error"] = list(demand_info["Actual Demand (MW)"] - wind_info["average_wind_forecasts"])

demand_info["net_demand_forecast_error"] = demand_info["demand_forecast_error"] - wind_info["wind_forecast_error"]
demand_info["demand_forecast_error_change"] = \
    demand_info["demand_forecast_error"].diff() / demand_info["demand_forecast_error"]

forecast_error_change_factor = demand_info["demand_forecast_error_change"].shift().iloc[3:]
...
factors_frame = pandas.DataFrame({"pre_price_growth": pre_price_growth,
                                  "forecast_error_change": forecast_error_change_factor,
...
factors_frame.reset_index(level=["Start Date", "Start Time 30 Minute Period"]).to_csv(".\\factors.csv", index=False)
```
...
py_run_string("factors_frame = r.pandas.read_csv('./factors.csv').set_index(['Start Date', 'Start Time 30 Minute Period'])")
py_run_string("factors_frame = factors_frame[['pre_price_growth', 'forecast_error_change', 'net_demand_growth', 'actual_price_growth_DAM', 'actual_price_growth_IDA1', 'actual_price_growth_BM']]")
...
ggplot(py$factors_frame, aes(x=time, y=pre_price_growth)) + geom_line(color="blue", show.legend = TRUE) + geom_line(aes(y=forecast_error_change),color="red", show.legend = TRUE) + geom_line(aes(y=net_demand_growth),color="yellow", show.legend = TRUE) + geom_line(aes(y=actual_price_growth_DAM),color="black", show.legend = TRUE) + geom_line(aes(y=actual_price_growth_IDA1),color="green", show.legend = TRUE) + geom_line(aes(y=actual_price_growth_BM),color="grey", show.legend = TRUE)+labs(y="factors")+theme(legend.position=c(0.8, 0.8))
```
&nbsp;
In the above picture, the blue line denote growth rate in previous period, the red line denotes change rate of change rate of forecast error, the yellow line denotes the net demand growth rate, and the black, green, and greay ones denote the actual price growth rate in the three markets respectively. For the chart to be visualized in a way with more apparent pattern, extreme values with absolute value greater than 5 are abandoned before showing the chart.
...
forecast_error_change_factor = factors_frame["forecast_error_change"]
...
forecast_error_train = [list(forecast_error_change_factor)[i] for i in range(len(forecast_error_change_factor))
                        if i % 3 != 0]
forecast_error_test = [list(forecast_error_change_factor)[i] for i in range(len(forecast_error_change_factor))
                       if i % 3 == 0]
...
# reiterate factors
factors_train = [pre_price_train, forecast_error_train, net_demand_train]
factors_test = [pre_price_test, forecast_error_test, net_demand_test]
...
py_run_string("factors_frame = factors_frame[['pre_price_growth', 'forecast_error_change', 'net_demand_growth', 'actual_price_growth_IDA2']]")
py_run_string("useless_data = ['nan', 'inf', '-inf']")
py_run_string("factors_frame = factors_frame.iloc[[i for i in range(len(factors_frame.index)) if 'nan' not in list(map(lambda x: str(x), factors_frame.iloc[i])) and 'inf' not in list(map(lambda x: str(x), factors_frame.iloc[i])) and '-inf' not in list(map(lambda x: str(x), factors_frame.iloc[i])) and not [element for element in list(factors_frame.iloc[i]) if isinstance(element, float) and abs(element) > 5]]]")
py_run_string("dates = [index[0] for index in list(factors_frame.index)]")
py_run_string("times = [index[1] for index in list(factors_frame.index)]")
...
py$factors_frame$time <- strptime(py$factors_frame$time, "%d/%m/%Y%H:%M:%S")
ggplot(py$factors_frame, aes(x=time, y=pre_price_growth)) + geom_line(color="blue", show.legend = TRUE) + geom_line(aes(y=forecast_error_change),color="red", show.legend = TRUE) + geom_line(aes(y=net_demand_growth),color="yellow", show.legend = TRUE) + geom_line(aes(y=actual_price_growth_IDA2),color="green", show.legend = TRUE) + labs(y="factors")
```
&nbsp;
...
pandas <- import("pandas", convert=FALSE)
py_run_string("factors_frame = r.pandas.read_csv('./factors.csv').set_index(['Start Date', 'Start Time 30 Minute Period'])")
py_run_string("factors_frame = factors_frame[['pre_price_growth', 'forecast_error_change', 'net_demand_growth', 'actual_price_growth_IDA3']]")
...
py_run_string("factors_frame['time'] = xaxis")
py$factors_frame$time <- strptime(py$factors_frame$time, "%d/%m/%Y%H:%M:%S")
ggplot(py$factors_frame, aes(x=time, y=pre_price_growth)) + geom_line(color="blue", show.legend = TRUE) + geom_line(aes(y=forecast_error_change),color="red", show.legend = TRUE) + geom_line(aes(y=net_demand_growth),color="yellow", show.legend = TRUE) + geom_line(aes(y=actual_price_growth_IDA3),color="green", show.legend = TRUE) + labs(y="factors")
```
&nbsp;
