...
bear_market_stock = pandas.read_excel(".\\stock data.xlsx", sheet_name="个股熊市")
bear_market_stock["日期_Date"] = bear_market_stock["日期_Date"].apply(lambda x:
                                                                  datetime.date(x.year, x.month, 1))
bear_market_index = pandas.read_excel(".\\stock data.xlsx", sheet_name="创业板指数熊市").\
    rename(columns={"交易日期_TrdDt": "日期_Date"})
bear_market_index["日期_Date"] = bear_market_index["日期_Date"].apply(lambda x:
                                                                  datetime.date(x.year, x.month, 1))

bear_market_comparison = pandas.DataFrame()
for stock, group in bear_market_stock.groupby("股票代码_Stkcd"):
    bear_market_comparison = bear_market_comparison.append(pandas.merge(group, bear_market_index, how="outer",
                                                                        on="日期_Date").fillna(method="ffill"))

formation_months = [1, 3, 6, 9, 12]
test_months = [6, 6, 12, 12, 12]
total_bear_months = list(bear_market_comparison["日期_Date"].value_counts().index)
total_bear_months.sort()
total_months = len(total_bear_months)
...
for i_ft in range(5):
    ...
    for interval_start in range(total_months - interval_length + 1):
        bear_stock_car = pandas.DataFrame()
        bear_start_date = total_bear_months[interval_start]
        bear_end_date = total_bear_months[interval_start+interval_length-1]
        interval_bear_stock = bear_market_comparison.loc[(bear_market_comparison["日期_Date"] >= bear_start_date) &
                                                         (bear_market_comparison["日期_Date"] <= bear_end_date)]

        for stock, group in interval_bear_stock.groupby("股票代码_Stkcd"):
            group.sort_values(by="日期_Date")
            bear_cr = group["月收益率"].apply(lambda x: x+1).cumprod()
            bear_index_cr = group["指数月收益率_IdxMonRet"].apply(lambda x: x+1).cumprod()
            group["car"] = bear_cr - bear_index_cr
            if formation_month in [1, 3]:
                bear_stock_car = bear_stock_car.append(pandas.DataFrame({"stock_code": stock,
                ...
                bear_stock_car = bear_stock_car.append(pandas.DataFrame({"stock_code": stock,
                                                                         ...
                                                                         "twelfth_month_car": group["car"].iloc[
                                                                             formation_month + 11]},
                                                                        index=[bear_start_date]))
        bear_stock_car = bear_stock_car.sort_values(by=["formation_car"], inplace=False)
        best_stocks = bear_stock_car.iloc[-int(len(bear_stock_car) * 0.1):]
        worst_stocks = bear_stock_car.iloc[:int(len(bear_stock_car) * 0.1)]
...
output.to_csv(".\\bear market t values.csv", index=False)
factors_frame_complete.to_csv(".\\bear market factor values.csv", index=False)
